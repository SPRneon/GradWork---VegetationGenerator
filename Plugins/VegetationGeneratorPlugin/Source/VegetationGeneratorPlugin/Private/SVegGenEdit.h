...
class FVegetationGeneratorPluginEdMode;
...
class SVegGenEdit : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SVegGenEdit) {}
	SLATE_END_ARGS()
...
//	void NotifyVegGenTypeMeshChanged(UFoliageType* FoliageType);
//
////	/** Gets FoliageEditMode. Used by the cluster details to notify changes */
//	class FEdModeFoliage* GetVegGenEditMode() const { return VegGenEditMode; }
////
////	/** Will return the status of editing mode */
	bool IsVegGenEditorEnabled() const;
////	
////	/** Get the error message for this editing mode */
	FText GetVegGenEditorErrorText() const;
//
private:
	/** Creates the toolbar. */
...
private:
	...
	TSharedPtr<class SFoliagePalette> VegGenPalette;
	
	/** Current error message */	
	TSharedPtr<class SErrorText> ErrorText;

	/** Pointer to the foliage edit mode. */
	FVegetationGeneratorPluginEdMode*  VegGenEditMode;
};
